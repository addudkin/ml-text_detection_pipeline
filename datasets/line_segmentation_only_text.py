...
class LineSegOnlyText(
    ...
    def __init__(self,
        ...
        super(LineSegOnlyText, self).__init__(config, split)
        self.config = config
        self.split = split
        ...
        self.resize = A.Resize(height=1024, width=848, interpolation=2)
    ...
    def compare_two_rows(self, zero_row, first_row, width, highest, x_max):
        ...
        if (rest.shape[0] > 4) and (mode == 'max'):
        ...
        if rest.shape[0] > 0:
            ...
            first_dot = [new_line[:, 0].min(), new_line[0][1]]
            last_dot = [x_max, new_line[-1][1]]
        ...
        elif (rest.shape[0] == 0) and (mode == 'max'):
    ...
    def get_row_line_first(self, line_even, line_odd, width, x_max):
        ...
        ## even сверху
        new_line = self.compare_two_rows(even_row, odd_row, width, 'even', x_max)

        return new_line

    def get_row_line_second(self, line_even, line_odd, width, x_max):
        ...
        ## odd сверху
        new_line = self.compare_two_rows(even_row, odd_row, width, 'odd', x_max)
    ...
    def get_smoth_signle_bottom_line(self, line, width):
        ...
        new_line = np.array([[first_x, max_y], [last_x, max_y]])
        return new_line
    ...
    def create_mask(self, polys_even, polys_odd, image):
        ...
        if len(polys_even) != len(polys_odd):
            for even_line, odd_line in zip(polys_even[:-1], polys_odd):
                line = self.get_row_line_first(even_line, odd_line, width, X_MAX)
                full_lines.append(line)

            for even_line, odd_line in zip(polys_even[1:], polys_odd):
                line = self.get_row_line_second(even_line, odd_line, width, X_MAX)
                full_lines.append(line)
