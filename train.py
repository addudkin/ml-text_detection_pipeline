...
class Trainer:
    ...
    def evaluate_step(self, model: torch.nn.Module) -> None:
        ...
        self.check_current_result(model, self.eval_loss)# self.evaluator.result)

        # Print metrics for current epoch

        #show_metrics(self.evaluator.result, self.current_epoch, self.config['train']['batch_size'])
    ...
    def evaluate(
        ...
        with torch.no_grad():
            ...
            for i, (batch) in enumerate(loop):
                ...
                #
                # val_gt_polygons, _ = self.postprocessor(
                #     w, h, loss_inp['shrink_map'].unsqueeze(1), return_polygon=False
                # )
                #
                # val_polygons_after_step, val_scores_after_step = self.postprocessor(
                #     w, h, preds[:, :, :, :],return_polygon=False
                # )
                #
                # with torch.no_grad():
                #     if self.current_epoch % self.config.logging.log_images_every_n_epoch == 0 \
                #             and i == 0:
                #         #self.plot_result("val", i, batch, preds)
                #         self.plot_bbox_val("preds", batch[0], val_polygons_after_step, i)
                #         self.plot_bbox_val("gt", batch[0], val_gt_polygons, i)
                #
                # self.evaluator(val_gt_polygons, val_polygons_after_step)
    ...
    def _forward_step(
        ...
        """
        image_batch, shrink_maps_batch, shrink_masks_batch, threshold_maps_batch, threshold_masks_batch, _ = batch
