...
def get_masks(target_func, image: np.ndarray, polygons: List[List[np.ndarray]]) -> np.ndarray:
    ...
    for line in polys_even:
        even_polygons.extend(line)

    odd_polygons = []
    for line in polys_odd:
        odd_polygons.extend(line)

    polys_even = check_size_polys(even_polygons)
    polys_odd = check_size_polys(odd_polygons)

    shrink_maps_even, shrink_masks_even, threshold_maps_even, threshold_masks_even = target_func.create_target(
        image,
        polys_even
    )
    threshold_maps_even *= 255

    shrink_maps_odd, shrink_masks_odd, threshold_maps_odd, threshold_masks_odd = target_func.create_target(
        image,
        polys_odd
    )
    threshold_maps_odd *= 255

    even_target = np.array([shrink_maps_even, shrink_masks_even, threshold_maps_even, threshold_masks_even])
    odd_target = np.array([shrink_maps_odd, shrink_masks_odd, threshold_maps_odd, threshold_masks_odd])
    full_target = np.array([even_target, odd_target])
...
if __name__ == '__main__':
    ...
    workers = 12
    path2config_db = '/home/addudkin/ml_text_detection/configs/cism_even_odd_lines.yml'
