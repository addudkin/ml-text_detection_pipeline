...
class LCAU(nn.Module):
    def __init__(self, c, c_mid=64, scale=2, k_up=5, k_enc=3):
        ...
        """
        super(LCAU, self).__init__()
        self.scale = scale

        # self.comp = ConvBNReLU(c, c_mid, kernel_size=1, stride=1,
        #                       padding=0, dilation=1)
        self.enc = ConvBNReLU(c, 1, kernel_size=k_enc,
    ...
    def forward(self, X):
        ...
        W = self.enc(X)  # b * 100 * h * w
        # W = self.pix_shf(W)                             # b * 1 * h_ * w_
        W = self.upsmp(W)
        W = F.softmax(W, dim=1)  # b * 1 * h_ * w_
