...
class FPNDconv(nn.Module):
    def __init__(self, in_channels, inner_channels=256, **kwargs):
        ...
        print(in_channels)
        self.up_4 = nn.ConvTranspose2d(inner_channels, inner_channels, 2, 2)
        self.up_3 = nn.ConvTranspose2d(inner_channels, inner_channels, 2, 2)
        self.up_2 = nn.ConvTranspose2d(inner_channels, inner_channels, 2, 2)
        # reduce layers
    ...
    def forward(self, x):
        ...
        k = self.up_4(p5)
        print("Upsample_add: ", time.time() - current_time)
        p4 += k
        ...
        print("Upsample_cat: ", time.time() - current_time)
        x = self.conv(x)
        return x
...
class Truncated2FPN(nn.Module):
    def __init__(self, in_channels, inner_channels=256, **kwargs):
        ...
        super().__init__()
        inplace = True
        self.conv_out = inner_channels // 2
        inner_channels = inner_channels // 4
        # reduce layers
        self.reduce_conv_c2 = ConvBnRelu(in_channels[0], inner_channels, kernel_size=1, inplace=inplace)
        self.reduce_conv_c3 = ConvBnRelu(in_channels[1], inner_channels, kernel_size=1, inplace=inplace)
        ...
        self.out_channels = self.conv_out  # // 2
