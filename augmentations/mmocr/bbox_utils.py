...
from augmentations.mmocr.check_argument import is_2dlist, is_type_list
from augmentations.mmocr.point_utils import point_distance, points_center
...
def bbox2poly(bbox: np.ndarray, mode: str = 'xyxy') -> np.array:
    ...
    Args:
        bbox (np.ndarray): A bbox. In any form can be accessed by 1-D indices.
...
def bbox_center_distance(box1: np.ndarray, box2: np.ndarray) -> float:
...
def bbox_diag_distance(box: np.ndarray) -> float:
