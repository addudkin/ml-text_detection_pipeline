...
from augmentations.mmocr.bbox_utils import bbox2poly
from augmentations.mmocr.check_argument import valid_boundary
from augmentations.mmocr.check_argument import is_2dlist
from augmentations.mmocr.parce_poly import parce


def rescale_polygon(polygon: List,
...
def rescale_polygons(polygons: Sequence[List],
                     scale_factor: Tuple[int, int],
...
def poly2bbox(polygon: List) -> np.array:
...
def poly2shapely(polygon: List) -> Polygon:
...
def polys2shapely(polygons: Sequence[List]) -> Sequence[Polygon]:
...
def crop_polygon(polygon: List,
                 crop_box: np.ndarray) -> Union[List[np.ndarray], None]:
    ...
    if poly_cropped.area == 0. or not isinstance(
        ...
        if poly_cropped.area != 0:
            polygons = []
            if isinstance(poly_cropped, shapely.geometry.MultiPolygon):
                polygon = poly_cropped.convex_hull
                polygon = poly_make_valid(polygon)
                polygon = np.array(polygon.boundary.xy, dtype=np.float32)
                polygon = polygon.T
                # reverse poly_cropped to have clockwise order
                polygon = polygon[::-1, :].reshape(-1)
                polygons.append(polygon)
            if isinstance(poly_cropped, shapely.geometry.GeometryCollection):
                map = mapping(poly_cropped)['geometries']
                for ob in map:
                    if ob['type'] == 'Polygon':
                        polygon = Polygon(ob['coordinates'][0])
                        polygon = poly_make_valid(polygon)
                        polygon = np.array(polygon.boundary.xy, dtype=np.float32)
                        polygon = polygon.T
                        # reverse poly_cropped to have clockwise order
...
def is_poly_inside_rect(poly: List, rect: np.ndarray) -> bool:
...
def offset_polygon(poly: List, distance: float) -> List:
...
def sort_points(points):
    ...
    assert isinstance(points, np.ndarray) or is_2dlist(points)
